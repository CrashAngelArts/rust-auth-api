...
└── 🔹 `UserService`
    └── 🔸 `create_user`
├── 🔧 `change_password`
├── 🔧 `clear_recovery_code` - Limpa (remove) o código de recuperação do usuário.
├── 🔧 `delete_user`
├── 🔧 `generate_and_set_recovery_code` - Gera um novo código de recuperação persistente, faz o hash e o salva no banco.
├── 🔧 `get_user_by_email`
...
├── 🔧 `update_user`
├── 🔧 `verify_password`
├── 🔧 `verify_recovery_code` - Verifica se o código de recuperação fornecido corresponde ao hash armazenado.
